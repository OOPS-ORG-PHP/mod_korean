...
```bash
[root@host mod_korean-1.0.4]$ phpize
[root@host mod_korean-1.0.4]$ # --with-libdir 옵션은 64bit 환경에서만 지정합니다.
...
```bash
[root@host mod_korean-1.0.4]$ make test PHP_EXECUTABLE=/usr/bin/php
```

또는, install 전에 빌드한 extension을 직접 테스트 하려면 다음과 같이 가능 합니다.

```bash
[root@host mod_korean-1.0.4]$ php -d "extension_dir=./modules/" -d "extension=korean.so" some.php
```
...
```bash
[root@host mod_korean-1.0.4]$ make install
```

설치 시에, 테스트나 패키징을 위한 chroot 가 필요 하다면, 다음과 같이 사용 하십시오.

```bash
[root@host mod_korean-1.0.4]$ make install DESTDIR="/want/path"
```
