 ...
 * PHP 7 - [mod_korean 1.0.3](https://github.com/OOPS-ORG-PHP/mod_korean/releases/tag/1.0.3)
 * PHP 4 or PHP5 - [mod_korean 0.0.5](https://github.com/OOPS-ORG-PHP/mod_korean/releases/tag/0.1.5)

## Installation

```bash
[root@host mod_korean-1.0.3]$ phpize
[root@host mod_korean-1.0.3]$ # --with-libdir 옵션은 64bit 환경에서만 지정합니다.
[root@host mod_korean-1.0.3]$ ./configure --with-libdir=lib64 --enable-korean --enable-korean-gd=builtin
[root@host mod_korean-1.0.3]$ make
```
...
```bash
[root@host mod_korean-1.0.3]$ make test PHP_EXECUTABLE=/usr/bin/php
```

또는, install 전에 빌드한 extension을 직접 테스트 하려면 다음과 같이 가능 합니다.

```bash
[root@host mod_korean-1.0.3]$ php -d "extension_dir=./modules/" -d "extension=korean.so" some.php
```
...
```bash
[root@host mod_korean-1.0.3]$ make install
```

설치 시에, 테스트나 패키징을 위한 chroot 가 필요 하다면, 다음과 같이 사용 하십시오.

```bash
[root@host mod_korean-1.0.3]$ make install DESTDIR="/want/path"
```
