...
static int numberOfchar (char *str, char chk) {
	int no, i, len;
...
static unsigned int checkAddr (unsigned char * addr, int type) {
	regex_t preg;
	int regRet;
	...
	strcpy (regex, type ? regex_u : regex_e);
...
static unsigned int chkMetaChar (unsigned char * str, int type) {
	regex_t preg;
	int regRet;
	...
	strpy (regex, type ? regex_up : regex_ur);
...
static unsigned int check_table (unsigned char *str) {
	unsigned char * buf;
	unsigned char * regex[] =
	{
		";[\\d]+;",
	...
	buf = (unsigned char *) kr_regex_replace_arr (
...
static unsigned int multibyte_check (unsigned char *str_o, unsigned int p) {
 ...
 */
static unsigned int check_windows (unsigned int type) {
	switch (type) {
		case 1:
